<template>
  ...
  >
    <div class="max-w-4xl mx-auto relative">
      <!-- Decorator glow -->
      <div
        class="absolute -top-12 left-1/2 -translate-x-1/2 w-64 h-64 bg-indigo-500/20 dark:bg-indigo-500/10 rounded-full blur-3xl pointer-events-none"
      />
      ...
      >
        ...
        <div class="space-y-10 text-gray-700 dark:text-gray-300 leading-relaxed text-lg">
          <section v-for="(section, index) in sections" :key="index" class="group">
            <h2
              class="text-2xl font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-3 transition-colors group-hover:text-indigo-600 dark:group-hover:text-indigo-400"
            >
              <span
        ...
        <div class="mt-16 pt-8 border-t border-gray-100 dark:border-neutral-800 flex justify-center">
          <NuxtLinkLocale
            to="/"
            class="inline-flex items-center justify-center px-6 py-3 border border-transparent text-base font-medium rounded-xl text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors shadow-sm"
          >
...
defineOgImage({ alt: t('legal.privacy.title') })
